...
@pytest.mark.parametrize("version, exit_code", [(2, 0), (9, 1)])
...
"""
    ...
    if version == 2:
        ...
        assert "schema_version=9" in json.loads(stderr_lines[0])["error"]
        assert completed.stdout == ""
    assert cookie.read_bytes() == cookie_before
    with closing(sqlite3.connect(database)) as conn:
        assert conn.execute("SELECT version FROM schema_meta WHERE id=1").fetchone() == (8 if version == 2 else 9,)
