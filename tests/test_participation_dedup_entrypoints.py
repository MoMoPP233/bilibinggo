...
@pytest.mark.parametrize("version, exit_code", [(2, 0), (11, 1)])
...
"""
    ...
    if version == 2:
        ...
        assert "schema_version=11" in json.loads(stderr_lines[0])["error"]
        assert completed.stdout == ""
    assert cookie.read_bytes() == cookie_before
    with closing(sqlite3.connect(database)) as conn:
        assert conn.execute("SELECT version FROM schema_meta WHERE id=1").fetchone() == (10 if version == 2 else 11,)
