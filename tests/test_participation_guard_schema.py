...
def test_v2_to_v8_preserves_existing_business_data(isolated_home) -> None:
    _v2_database()
    before = _snapshot()
    schema.init_db()
    schema.init_db()
...
def test_v1_jobs_migration_continues_through_v8(isolated_home) -> None:
    _v2_database()
    with get_engine().begin() as conn:
...
def test_future_version_rejected_before_any_create_all(isolated_home, monkeypatch) -> None:
    _v2_database()
    with get_engine().begin() as conn:
        conn.exec_driver_sql("UPDATE schema_meta SET version=9")
    before = _snapshot()
    ...
    with pytest.raises(RuntimeError, match="schema_version=9"):
        schema.init_db()
    assert _snapshot() == before
    with get_engine().connect() as conn:
        assert conn.exec_driver_sql("SELECT version FROM schema_meta").scalar_one() == 9
